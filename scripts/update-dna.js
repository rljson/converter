...
const filesToBeDeleted = [
  ...
  'doc/workflows/update-from-converter.md',
];

const templateRepo = 'https://github.com/rljson/converter.git';
const localRepoPath = path.resolve(__dirname, '../../converter');
...
function replaceConverter() {
  ...
  for (const file of replaceFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/converter/g, projectName);
